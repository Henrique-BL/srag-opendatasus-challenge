...
def _build_report(state: ReportState):
    ...
    # Title
    doc.preamble.append(Command('title', NoEscape(r'\textbf{RELATÓRIO TÉCNICO DE SAÚDE PÚBLICA - INFLUENZA}')))
        ...
        with doc.create(Subsection("Relação Mensal e Anual")):
            ...
            with doc.create(Figure(position="H")) as fig:
                for caption, label, graphic in [
                    ("Análise Mensal", "fig:casos-30-dias", "../graphics/monthly-analysis.png"),
                    ("Análise Anual", "fig:casos-12-meses", "../graphics/yearly-analysis.png"),
                ]:
                    with doc.create(MiniPage(width=NoEscape("0.45\\textwidth"))) as mini:
                        mini.append(NoEscape(r"\centering"))
                        mini.append(NoEscape(f"""\includegraphics[width=\\textwidth]{{{graphic}}}"""))
                        mini.append(NoEscape(fr"\caption{{{caption}}}"))
                        mini.append(NoEscape(fr"\label{{{label}}}"))
...
def _create_graphics(state: ReportState) -> ReportState:
    ...
    try:
        ...
        if monthly:
            ...
            path = _create_and_save_chart(df_monthly, "monthly-analysis", "Análise Diária")
            graphics_paths.append(path)
        
        if one_year_interval:
            df_yearly = _get_dataframe(one_year_interval)
            path = _create_and_save_chart(df_yearly, "yearly-analysis", "Análise Mensal - 1 ano")
            graphics_paths.append(path)
...
def _get_srag_news(state: ReportState) -> ReportState:
    try:
        ...
        news = asyncio.run(search_tool.ainvoke({"start_date": start_date, "end_date": end_date}))
        state["news"] = news
        return state
    except Exception as e:
...
def _get_srag_data(state: ReportState) -> ReportState:
    try:
        ...
        all_years = query_tool.invoke({"data_to_fetch": "all", "group_by": "year"})
        monthly = query_tool.invoke({"data_to_fetch": "total_cases",
                                     "start_date": (report_date - relativedelta(months=1)).strftime("%Y-%m-%d"), "end_date": report_date.strftime("%Y-%m-%d"), "group_by": "day"})
        one_year_interval = query_tool.invoke({"data_to_fetch": "total_cases",
                                               "start_date": (report_date - relativedelta(years=1)).strftime("%Y-%m-%d"), 
                                               "end_date": report_date.strftime("%Y-%m-%d"), "group_by": "month"})
        state["data"] = {"all_years": all_years,
                         "monthly": monthly,
